...
public class AudioBook extends AudioContent
{
  ...
  // Print information about the audiobook. First print the basic information of the AudioContent 
	...
	{
		super.printInfo(); //prints basic info about audiobook
		System.out.println("Author: " + this.author + " Narrated by: " + this.narrator); //prints author and narrorator
	}
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	public void play()
	{
		super.setAudioFile(chapterTitles.get(currentChapter) + "." + "\n" + chapters.get(currentChapter)); //sets audio file to current chapter
		super.play();//calls play method in super class 
	}
	...
	{
		//loop which goes through list of chapter titles
		for(int i = 0; i < chapterTitles.size(); i++)
		{
			System.out.println("Chapter " + (i+1) + ". " + chapterTitles.get(i));//prints chapter number and title
			System.out.println("");
		}
	}
	...
	{
		AudioBook book2 = (AudioBook) other; //create new audiobook 
		//returns true if the audiobooks are equal (same author, narrorator, book), false if not
		return (this.getAuthor()).equals(book2.getAuthor()) && (this.getNarrator()).equals(book2.getNarrator()) && super.equals(other); 
	}
	...
	//return chapter title list
	public ArrayList<String> getChapterTitles()
	{
		return chapterTitles;
	}
	//set chapter titles list
